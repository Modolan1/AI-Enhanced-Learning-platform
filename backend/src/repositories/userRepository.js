...
export const userRepository = {
  ...
  findInstructorById: (id) => User.findOne({ _id: id, role: 'instructor' }).select('-passwordHash').populate('requestedCourse', 'title category level'),
  findStudents: () => User.find({ role: 'student' }).select('-passwordHash').sort({ createdAt: -1 }),
  findInstructors: () => User.find({ role: 'instructor' }).select('-passwordHash').populate('requestedCourse', 'title category level').sort({ createdAt: -1 }),
